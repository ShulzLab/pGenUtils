...
from PyQt5.QtWidgets import QSlider, QPushButton, QLineEdit, QLabel, QGroupBox, QHBoxLayout, QVBoxLayout, QGridLayout, QCheckBox, QWidget, QApplication, QStyleFactory
...
sys.path.append(r"C:\Users\Timothe\NasgoyaveOC\Professionnel\TheseUNIC\Scripts\Python")


from LibrairieQtDataDive.legacy import MyMplCanvas
...
class VisualizeGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.sc = MyStaticMplCanvas(Group, width=5, height=4, dpi=100)
        ...
        self.LoadInRAM.stateChanged.connect(self.ChangeRamTodisk)
    ...
    def UpdateFigures(self):
        ...
        self.sc.update_figure(self.FindImage(), Vauto = True)
    ...
    def ChangeRamTodisk(self):

        if self.LoadInRAM.isChecked() == False :
            self.Frameslider.valueChanged.disconnect()

            self.Frameslider.valueChanged.connect(self.UpdateFrameIDLabel)
            self.Frameslider.sliderReleased.connect(self.UpdateFigures)

        else :
            self.Frameslider.valueChanged.disconnect()
            self.Frameslider.sliderReleased.disconnect()
    ...
    def FindImage(self):
        ...
        if self.LoadInRAM.isChecked() == True :
            Location = 'RAM'
        else :
            Location = 'Disk'
        ...
        if Location == 'RAM':
            ...
            Image = self.FrameArray[:,:,self.Frameslider.value()]

        elif Location == 'Disk':

            self.FrameArray.set(cv2.CAP_PROP_POS_FRAMES, int(self.Frameslider.value()))
            _ , Image = self.FrameArray.read()

        else :
            raise ValueError("Data location to point is neither RAM or Disk : can't acess data, but data may be there")
    ...
    def CheckDataIsok(self):

        if self.VSDisok and self.Behisok :
